...
function AfForm(container)
{
	...
	// 返回的是Element对象
	this.getElementByName = function ( elementName )
	{
		var eee = $('[name]',this.div); // 获取所有有name属性的元素
		for(var e of eee)
		{
			var a = $(e).attr('name');
			if( a == elementName) return e;
		}
		return null;
	};
...
function AfDialog(container)
{
	...
	this.set = function( obj )
	{
		new AfForm(this.div).set(obj);	
	};
	
	this.get = function( )
	{
		return new AfForm(this.div).get();
	}
	
	this.child = function(selector)
	{
		var e =  $(selector, this.div);
		if(e.length == 0)
			throw "你的selector给错了! (selector: '" + selector + "')";
		return e;
	}
	
	this.getMask = function()
	{
		return this.div.dialog_mask;
	};
	...
	this.showMask = function()
	{
		...
		if(mask == null)
		{
			...
			mask.setAttribute('class', 'af-dialog-mask');
			document.body.appendChild(mask);	
			this.div.dialog_mask = mask; // 将此mask存在对话框DOM元素下，以便下次加载的时候还能找到		
		}

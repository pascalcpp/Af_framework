...
public class GenerateActivity extends AfActivity
{
	...
	// 框架类型: afsql, mybatis
	JComboBox<String> apiTypeField = new JComboBox<>();
	...
	public JComponent initCenter()
	{
		...
		JLabel a2 = new JLabel("数据库API类型：");
		a2.setFont(a2.getFont().deriveFont(Font.PLAIN));
		panel.add(a2, "30px");
		panel.add(apiTypeField, "30px");
		
		apiTypeField.addItem("afsql");
		apiTypeField.addItem("general");
	...
	public void onStart()
	{
		...
		// API类型
		if(info.apiType.equals("afsql"))
			apiTypeField.setSelectedItem("afsql");
		else
			apiTypeField.setSelectedItem("general");
	...
	private void onGenerate()
	{
		...
		info.apiType = (String)apiTypeField.getSelectedItem();
		info.save();

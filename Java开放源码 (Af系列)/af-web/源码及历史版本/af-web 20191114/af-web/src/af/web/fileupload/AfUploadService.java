...
public class AfUploadService extends HttpServlet
{
	...
	{		
		...
		{
			throw new RuntimeException("af-service.xml: " + e.getMessage());
		}
	}
			...
			HttpServletResponse response) throws ServletException, IOException
	{
		...
		try
		{
			...
			AfUploadHandler instance = null;
			try{
				instance = (AfUploadHandler) cfg.clazz.newInstance();				
	...
	private Object doUpload(HttpServletRequest request, 
			...
			AfUploadHandler handler) throws Exception
	{		
		...
		AfFormData formData = new AfFormData();	
	...
	{		
		...
		if(stream == null)
			throw new Exception("找不到 af-service.xml,请确保有此配置文件!");
		...
		for (Element e : xServiceList)
		{
			...
			// 检查是不是 AfUploadHandler 的子类
			if(cfg.clazz.isAssignableFrom( AfUploadHandler.class))
				throw new Exception("类" + cfg.clazzName + "不是AfUploadHandler的子类!无法加载!");
